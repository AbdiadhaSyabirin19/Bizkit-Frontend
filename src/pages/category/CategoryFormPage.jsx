    ...
    <Layout title={isEdit ? 'Edit Kategori' : 'Tambah Kategori'}>
      <div className="max-w-lg mx-auto">
        ...
        <div className="flex items-center gap-3 mb-6">
          <button onClick={() => navigate('/categories')} className="p-2 hover:bg-gray-100 rounded-lg transition">
            <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          </button>
          <div>
            <h1 className="text-xl font-bold text-gray-800">{isEdit ? 'Edit Kategori' : 'Tambah Kategori'}</h1>
            <p className="text-gray-500 text-sm">{isEdit ? 'Perbarui data kategori' : 'Tambah kategori baru'}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3 pb-6">
            <button onClick={() => navigate('/categories')} className="flex-1 px-4 py-3 border border-gray-200 rounded-xl text-gray-600 hover:bg-gray-50 text-sm font-medium transition">Batal</button>
