...
export default function OutletPage() {
  ...
  const columns = [
      ...
      render: (row) => (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {row.address && <p className="text-xs text-gray-400 dark:text-zinc-500">{row.address}</p>}
          </div>
        </div>
      )
    },
    {
      ...
      render: (row) => <span className="text-sm text-gray-600 dark:text-zinc-400">{row.phone || '-'}</span>
    },
    {
